...
export const MessageCard = ({ message }: { message: Message }) => {
            ...
            const observer = new IntersectionObserver(entries => {
                entries.forEach(entry => {
                    if (entry.isIntersecting) {
                        messageRefCurrent.classList.add('animate-in', 'fade-in', 'slide-in-from-left');
                        messageRefCurrent.classList.remove('opacity-0');
                    }
                });
            }, { threshold: 0.2 });

            observer.observe(messageRefCurrent);

            return () => {
                observer.unobserve(messageRefCurrent);
            };
        }
    }, []);
    ...
    return (
        <div
            className='flex w-full gap-3 p-4 duration-300 opacity-0 sli'
            ref={messageRef}
        >
            <div className='flex flex-col'>
                <Avatar>
                    <AvatarFallback className='bg-violet-500'>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-full h-full translate-y-1 fill-blue-300 stroke-blue-300">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
                        </svg>
                    </AvatarFallback>
                </Avatar>
                <p className='opacity-70'>{timeString}</p>
            </div>
            <div className='flex flex-col w-full gap-3'>
                <div className='flex justify-between'>
                    <div>
                        <p className='opacity-70'>{message.author}</p>
                        <p className='italic opacity-70'>{message.channel}</p>
                    </div>
                    <div className='flex gap-7'>
                        ...
                        <div className='flex items-center gap-2'>
                            <svg width="25" height="25" viewBox="0 0 18 15" xmlns="http://www.w3.org/2000/svg" className='cursor-pointer fill-blue-500 hover:stroke-blue-500/80'>
                            ...
                            <svg width="27" height="27" viewBox="0 0 24 24" fill="none" className='cursor-pointer stroke-blue-500 hover:fill-blue-500/40' xmlns="http://www.w3.org/2000/svg">
                                <rect x="3.625" y="3.625" width="16.75" height="16.75" strokeWidth="1.25" />
                                <line x1="3" y1="8.375" x2="21" y2="8.375" strokeWidth="1.25" />
                            </svg>
                            <svg width="26" height="27" viewBox="0 0 20 21" fill="none" className='cursor-pointer fill-blue-500 hover:stroke-blue-500/80' xmlns="http://www.w3.org/2000/svg">
                            ...
                            <FavoriteButton MessageData={message} />
                        </div>
                    </div>
                </div>
                <div>
                    <p>{message.content ? message.content : 'без текста'}</p>
                    <Button className='p-0 opacity-70' variant={'link'} size={'sm'}>Далее</Button>
                    <div>
                        {message.attachments?.length !== undefined && message.attachments?.map((item) => (
                            item.type === 'video' ?
                                <video key={item.url} width="640" height="360" controls>
                                    <source src={item.url} type="video/mp4" />
                                </video>
                                :
                                <Image className='border rounded' key={item.url} src={item.url} height={260} width={150} alt={item.url} />
                        ))}
                    </div>
                    <div className='flex gap-2 mt-2 text-sm'>
                        <p className='text-blue-500'>&#35;Новое</p>
                        <p className='opacity-40'>&#35;Эксперт</p>
                    </div>
